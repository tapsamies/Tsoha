        ...
        if not user:
            ...
            if check_password_hash(user.password, password):
                ...
                return render_template(
                    "error.html", message="Tunnus tai salasana väärin"
                )
...
def send():
    ...
    sql = text(
        "INSERT INTO messages (content, user_id, sent_at) VALUES (:content, :user_id, NOW())"
    )
...
def board():
    result = db.session.execute(
        text(
            "SELECT M.content, U.username, M.sent_at FROM messages M, users U WHERE M.user_id=U.id"
        )
    )
    messages = result.fetchall()
...
def boards():
    result = db.session.execute(
        text("SELECT id, name FROM categories WHERE public=TRUE")
    )
